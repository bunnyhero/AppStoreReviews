...
wr = csv.DictWriter(
    resultsFile,
    ["topic", "review", "version", "user", "rank"],
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="AppStoreReviewsScrapper command line.",
    ...
    parser.add_argument(
        "-l", "--list", action="store_true", default=False, help="AppStores list"
    )
    parser.add_argument(
        "-m",
        "--max-reviews",
    ...
    parser.add_argument(
        "-r", "--raw-mode", action="store_true", default=False, help="Print raw mode"
    )
    ...
    if args.list:
        ...
        if country == "All":
            ...
            for c in countries:
                ...
                rankCount += rc
                rankSum += rs
            print "\nTotal number of reviews: %d, avg rank: %.2f" % (
                rankCount,
                1.0 * rankSum / rankCount,
            )
        else:
            try:
                reviews = getReviews(
